SELECT
  ...
  sum(rtds) * 6 + sum(xprushconv) * 2                                                   AS points,
  round(((sum(rtds) * 6 + sum(xprushconv) * 2) / (sum(game) + 0.000001)) :: NUMERIC, 2) AS ppg,
  sum(ratts)                                                                            AS ratts,
  sum(ruyds)                                                                            AS ruyds,
  round((sum(ruyds) / (sum(ratts) + 0.000001)) :: NUMERIC, 2)                           AS rattsperattm,
  round((sum(ruyds) / (sum(game) + 0.000001)) :: NUMERIC, 2)                            AS yardspergame,
  sum(rtds)                                                                             AS rtds,
  round((sum(ratts) / (sum(game) + 0.000001)) :: NUMERIC, 2)                            AS rattspergame,
  sum(xprushconv)                                                                       AS xprushconv,
  sum(firstdbrush)                                                                      AS firstdbrush
