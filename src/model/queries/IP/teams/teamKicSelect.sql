SELECT
  ...
  sum(fgm) * 3 + sum(xpkickconv)                                                   AS points,
  round(((sum(fgm) * 3 + sum(xpkickconv)) / (sum(game) + 0.000001)) :: NUMERIC, 2) AS ppg,
  sum(fga)                                                                         AS fgattm,
  sum(fgm)                                                                         AS fgmade,
  sum(xpkickatt)                                                                   AS patt,
  sum(xpkickconv)                                                                  AS pattm,
  sum(fgblocked)                                                                   AS blockedfg,
  sum(xpblocked)                                                                   AS blockedxp,
  sum(punt)                                                                        AS punts,
  sum(puntblocked)                                                                 AS blockedpunt
